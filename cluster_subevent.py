'''
Clusters the subevent features from extract_subevent_features.py. Uses pairwise cosine distance and min-linkage as the distance function. Implemented using a Kruskal-like algorithm.

Expected constants defined:
        ANNOTATION_DIR: used to access names of classes
        BOW_MATRIX: same from extract_subevent_features.py
        YAGO_MATRICES: same from extract_subevent_features.py
        DB_MATRICES: same from extract_subevent_features.py
        PAIRS: output directory for pairwise distances intermediate data (to save time in future computation)
        BOW_CLUSTERS: directory for storing clusters using only BOW data
        BOW_YAGO_CLUSTERS: '' using B+Y data
        BOW_YAGO_DB_CLusters: '' using B+Y+D data

Note: we ended up not using clustering for the final data.
'''
